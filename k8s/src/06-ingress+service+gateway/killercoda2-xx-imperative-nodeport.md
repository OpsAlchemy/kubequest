...
controlplane:~$ k expose deployment/nginx-app-cka --name=app-service-cka --port=80 --protocol=TCP --target-port=80 --
node-port=31000 --type=NodePort
error: unknown flag: --node-port
See 'kubectl expose --help' for usage.
controlplane:~$ k expose -h | grep node
...
controlplane:~$ k expose -h | grep -A3 -B3 node
...
Options:
    ...
    --external-ip='':
        Additional external IP address (not managed by Kubernetes) to accept for the service. If
        this IP is routed to a node, the service can be accessed by this IP in addition to its
...
error: unknown shorthand flag: 'd' in -dry-run=client
See 'kubectl expose --help' for usage.
controlplane:~$ k expose deployment/nginx-app-cka --name=app-service-cka --port=80 --protocol=TCP --target-port=80  --type=NodePort --dry-run=client -o yaml > svc.yaml
Error from server (NotFound): deployments.apps "nginx-app-cka" not found
controlplane:~$ k expose deployment/nginx-app-cka --name=app-service-cka --port=80 --protocol=TCP --target-port=80  --type=NodePort -n nginx-app-space --dry-run=client -o yaml > svc.yaml
